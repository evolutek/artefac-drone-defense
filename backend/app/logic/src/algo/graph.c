...
Node** to_graph(ClusterIndex* cluster_indices, uint16_t nb_cluster) {
    ...
    for (uint16_t i_clt = 0; i_clt < nb_cluster; i_clt++) {
		...
		n_wh_darr[i_clt] = pool_query(&ctx.node_pool, idx_wh);
...
void link_intra_archetype(Archetype *at, Node *wh, uint32_t *i_wh_edge) {
    ...
    for (size_t i1_del = 0; i1_del < nb_del; i1_del++) {
        ...
        wh->edges[*i_wh_edge++] = pool_query(&ctx.edge_pool, idx_edge);
...
void link_deliveries(Delivery* del1, Delivery* del2) {
    ...
    if (del1->user_priority <= del2->user_priority) {
        ...
        (del1->node)->edges[(del1->node)->nb_edges++] = pool_query(&ctx.edge_pool, idx_edge1);

        if (del1->user_priority == del2->user_priority) {
			pool_add(&ctx.edge_pool, Edge, link(del2->position, del1), idx_edge2);
            (del2->node)->edges[(del2->node)->nb_edges++] = pool_query(&ctx.edge_pool, idx_edge2);
		}
    } else {
		pool_add(&ctx.edge_pool, Edge, link(del2->position, del1), idx_edge2);
        (del2->node)->edges[(del2->node)->nb_edges++] = pool_query(&ctx.edge_pool, idx_edge2);
