...
int main(void) {
    ...
    // Initialize interface with backend (via shared memory)
    if (!init_interface()) {
        fprintf(stderr, "WARNING: Failed to initialize interface. Make sure backend is running.\n");
        fprintf(stderr, "         Continuing in standalone test mode...\n");
    }
